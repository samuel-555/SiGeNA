...
<%
    ...
    if (cargo != null) {
        switch (cargo) {
            case GERENTE ->
                cargoIcon = "IMG's/Identificador-gerente.png";
            case ZOOTECNISTA ->
                cargoIcon = "IMG's/Identificador-zootecnista.png";
            case VETERINARIO ->
                cargoIcon = "IMG's/Identificador-veterinario.png";
            case TRATADOR ->
                cargoIcon = "IMG's/Identificador-tratador.png";
            default ->
                cargoIcon = null;
        }
    }
    ...
    features.add(new Feature("enriquecimento", "Gestão de Enriquecimento", "enriquecimento"));
    features.add(new Feature("visitantes", "Gestão de Visitantes", "visitantes"));
            ...
            <div class="user-area">
                <div class="notif-wrapper">
                    ...
                    <button class="notif-button" onclick="toggleNotifs()">
                        ...
                        <c:if test="${not empty notiicacoes}">
                        ...
                        <div class="notif-body">
                            <c:forEach var="n" items="${notificacoes}">
                                <div class="notificacao ${n.lida ? 'lida' : 'nao-lida'}">
                                    ...
                                    <c:if test="${!n.lida}">
                                        <form id="form-${n.id}" action="HomeController" method="post">
                                            <input type="hidden" name="acao" value="lerNotificacao">                            
            ...
            <section class="tarefas-section">
                <div class="tarefas-header">
                    ...
                    <c:if test="${sessionScope.cargoUsuario.name() eq 'GERENTE'}">
                        <a href="HomeController?acao=cadastrar" class="btn-primary">Criar Nova Tarefa</a>
                    </c:if>
                </div>
                                            ...
                                            <td style="text-align:right">
                                                <c:choose>
                                                    <c:when test="${!tarefa.concluida}">
                                                        <c:if test="${sessionScope.cargoUsuario.name() eq 'GERENTE'}">
                                                            <a href="HomeController?acao=editar&id=${tarefa.id}" class="btn-action editar">Editar</a>
                                                            <form action="HomeController" method="POST" style="display:inline;" onsubmit="return confirm('Excluir esta tarefa?')">
